...
const getChannelId = async (userId) => {
  const page = await fetch(`https://osu.ppy.sh/community/chat?sendto=${userId}`).then(res => res.text());
  const html = new DOMParser().parseFromString(page, 'text/html');
  const json = JSON.parse(html.querySelector('script#json-chat-initial').textContent);
  const presence = json.presence;
  const channelId = presence.find(p => p.type === 'PM' && p.users[0] === parseInt(userId));
...
if (location.pathname.match(/\/rankings\/[a-z]+\/[performance|score]/g)) {
  ...
  content.appendChild(createMassDmButton(document.querySelectorAll('[data-user-id]').length));
  gridItem.appendChild(content);
  document.querySelector('.osu-page.osu-page--ranking-info .grid-items').appendChild(gridItem);
}

/*
const users = document.querySelectorAll('.ranking-page-table__user-link-text');
users.forEach(user => {
const id = user.getAttribute('data-user-id');
console.log(id);
});*/
sendDm('25562290', 'flop');